...
\begin{hostbashinput}
$ cd $HOME/__SESSION_NAME__-labs/buildroot
...
room in its \code{net_device.priv} field, we can pass the size of the private data in the \code{rtnl_link} info :
 ...
 We now need to populate the 3 callback functions that were mentioned : \code{bootlinlan_setup}, \code{bootlinlan_newlink} and \code{bootlinlan_dellink}.
 ...
 \begin{itemize}
 	\item Specify the \kstruct{net_device} information about the encapsulation (MTU, header size, etc). In our case, we can simply reuse the ones from a regular ethernet device by calling \kfunc{ether_setup}
 ...
 \begin{itemize}
	 ...
	 \item \code{struct nlattr *data[]} : A Netlink Attribute array containing attributes specific to our link type
