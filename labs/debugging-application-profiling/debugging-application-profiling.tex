...
\ifdefstring{\board}{beagleplay}{ You will quickly see that perf does
...
\begin{bashinput}
$ perf stat -e %\ifdefstring{\board}{beagleplay}{L1-dcache-load-misses}{L1-dcache-store-misses}% ./png_convert tux.png out.png
\end{bashinput}
...
build id that is running on the board or perf will not use it. You can
verify those build IDs in both perf.data and vmlinux file with perf:

- for perf.data: perf buildid-list (in the directory containing your
perf.data file)

- for vmlinux: perf buildid-list -i <your\_vmlinux\_file>

If IDs mismatch, you need to re-sync your running kernel with the one in
