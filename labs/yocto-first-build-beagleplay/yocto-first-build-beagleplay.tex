...
Then download the \code{meta-arm} and \code{meta-ti} layers:
\begin{bashinput}
git clone https://git.yoctoproject.org/meta-arm
cd meta-arm
git checkout 0979b8042909
cd ..
git clone -b 12.01.00.05 https://git.yoctoproject.org/meta-ti
cd meta-ti
git am $HOME/__SESSION_NAME__-labs/bootlin-lab-data/0001-Don-t-use-a-custom-deployment-directory.patch \
  $HOME/__SESSION_NAME__-labs/bootlin-lab-data/0002-Use-extlinux.conf-booting-flow.patch \
  $HOME/__SESSION_NAME__-labs/bootlin-lab-data/0003-u-boot-enable-saving-the-env-in-fat-partition.patch
\end{bashinput}
...
Don't forget to make the configuration aware of the meta-poky, ARM and TI layers.
...
\code{meta-ti/meta-ti-bsp},
and \code{meta-ti/meta-beagle}
...
\begin{bashinput}
sudo bmaptool copy --removable-device $BUILDDIR/tmp/deploy/images/beagleplay/core-image-minimal-beagleplay.rootfs.wic.xz /dev/mmcblkX
\end{bashinput}
