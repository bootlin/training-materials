\subchapter{Lab 3: Add a custom application}{Add a new recipe to support a required
	custom application}
...
\begin{itemize}
  ...
  \item The internal state has changed, clean the working directory: \\
     \code{bitbake -c clean ninvaders}
\end{itemize}

\subsection{Conflicting types error}

One of the build failures you will face will complain about conflicting
definitions of \code{doSleep}: the definition having a \code{void(void)}
prototype, while the definition uses \code{void(int)}.

This is because of a new behaviour with C23. We can revert to an old standard
version, where \code{void()} prototypes do not impose any restriction on
provided parameters. This can be done using the \code{-std=gnu17} {\em gcc}
flag.

To achieve this, make sure to add \code{-std=gnu17} to the \yoctovar{CFLAGS}
variable.

\subsection{Multiple definition error}

Another build failure you will face will generate many messages such as
