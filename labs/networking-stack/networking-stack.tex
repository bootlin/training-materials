...
The "UP" keyword appears as the last element between the chevrons following the interface
name, this is the admin link state. When the interface is "admin down", no keyword will appear in that spot.
...
We'll reuse what we have configured so-far with our namespaces, let's just flush the IP address from \code{lan0} :
