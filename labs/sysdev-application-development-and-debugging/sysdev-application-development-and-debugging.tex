...
\begin{bashinput}
$ export PKG_CONFIG_PATH=$HOME/__SESSION_NAME__-labs/integration/buildroot/output/host/%\ifdefstring{\arch}{ARM64}{aarch64-buildroot-linux-gnu}{arm-buildroot-linux-gnueabihf}%/sysroot/usr/lib/pkgconfig
$ %\ifdefstring{\arch}{ARM64}{aarch64-linux-gcc}{arm-linux-gcc}% -o nunchuk-mpd-client nunchuk-mpd-client.c \
...
\begin{itemize}
  \item If you don't have an SSH key yet (look at \code{~/.ssh/}),
	...
	\code{id_ed25519} (private key) and \code{id_ed25519.pub} (public key).
  \item Then create a \code{/root/.ssh/authorized_keys} file {\bf on the target}
        with the line in \code{id_ed25519.pub}.
...
By studying the the code, you should eventually find that what's causing
