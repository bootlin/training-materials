\subchapter{Lab 5: Extend a recipe}{Add your features to an existing recipe}
...
\ifdefstring{\board}{bbb}{
  \def\linuxrecipename{linux-bb.org}
}{}

\ifdefstring{\board}{stm32mp1}{
  \def\linuxrecipename{linux-stm32mp}
}{}
...
\ifdefstring{\board}{imx93-frdm}{
  \def\linuxrecipename{linux-fslc-imx}
}{}

We here aim to extend the {\tt \linuxrecipename{}} kernel recipe.
...
\if\defstring{\board}{stm32mp1}
\begin{verbatim}
linux-stm32mp_6.6.bb:
  $HOME/__SESSION_NAME__-labs/meta-bootlinlabs/recipes-kernel/linux/linux-stm32mp_6.6.bbappend
\end{verbatim}
\fi
...
\begin{verbatim}
linux-fslc-imx_6.6.bb:
  $HOME/__SESSION_NAME__-labs/meta-bootlinlabs/recipes-kernel/linux/linux-fslc-imx_6.6.bbappend
\end{verbatim}
\fi

\section{Add patches to apply in the recipe}

We want our extended {\tt \linuxrecipename{}} kernel to support the Nunchuk as
...
\fi
\if\defstring{\board}{imx93-frdm}
Add \code{KBUILD_DEFCONFIG:imx93-11x11-lpddr4x-frdm = ""} to allow the use of our \code{defconfig} file.
\fi
\if\defstring{\board}{beagleplay}
It is handled automatically in the \code{linux-bb.org} original recipe.
\fi
\if\defstring{\board}{bbb}
It is handled automatically in the {\code{linux-bb.org} original recipe.
\fi
...
We will connect it to the \ifdefstring{\board}{beagleplay}{\code{i2c3}}{\code{i2c1}} port
...
Connect the Nunchuk pins:
\begin{itemize}
\item The \code{GND} pin to P11 pin 6 (\code{GND})
\item The \code{PWR} pin to P11 pin 1 (\code{3V3})
\item The \code{SDA} pin to P11 pin 3 (\code{SDA})
\item The \code{SCL} pin to P11 pin 5 (\code{SCL})
\end{itemize}

\begin{center}
\includegraphics[width=12cm]{common/imx93-frdm-connect-nunchuk.jpeg}
\end{center}
}{}
