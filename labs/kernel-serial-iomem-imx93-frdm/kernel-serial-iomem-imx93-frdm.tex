...
\code{imx93-frdm-custom} branch, this way we can keep the same custom Device
...
declarations for LPUART3 and LPUART8.
...
\kfile{arch/arm/boot/dts/freescale/imx93.dtsi}. In the above code, we just
...
value at a given register offset fromthe device base virtual address.
...
\sourcecode{labs/kernel-serial-iomem-imx93-frdm/uart-line-init.c}

Dont forget to declare \code{baud_val} in probe function 

We are now ready to transmit characters over the serial ports!

If you have a bit of spare time, you can look at chapiter 60 of the
imx93 processor reference manual for details about how to use the LPUART ports, to understand
...
(just connect to the UART8 pins instead of the UART3 ones).
