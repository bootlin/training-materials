...
{Objectives:
  \begin{itemize}
	  \item Prepare the \boarddescription{} board
  \end{itemize}
}
...
The rootfs should be extracted at \code{/home/$USER/__SESSION_NAME__-labs/nfsroot}
using this command:
...
\begin{bashinput}
$ export CROSS_COMPILE=/home/$USER/__SESSION_NAME__-labs/buildroot/output/host/bin/%\toolchainprefix%-
\end{bashinput}
...
\ifdefstring{\board}{beagleplay}{\textbf{Warning: take care that if you also have
some valid software flashed in the on-board eMMC, the board will boot it by
default ! To force the bootrom to select the SD Card as default boot
support, make sure to press the "USR" button while applying power to the
board. You only need to do this on "cold boots": the board will keep using
the SD card as default boot medium if you press the reset button or enter
"reboot" in Linux.}}{}
