...
\item One partition, for the root filesystem, 32 MB big:
\if\defstring{\board}{stm32mp1}
\begin{verbatim}
mkpart rootfs 131072s 196604s
\end{verbatim}
\fi
...
As we are going to use SquashFS to store the root file system, we will no longer
be able to write files uploaded through the web interface on it. As an
alternative, we are going to create a data partition that will contain these
files.
