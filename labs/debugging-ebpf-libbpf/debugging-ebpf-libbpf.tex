...
\begin{itemize}
    \item Since our program deals with pt\_regs, it is not portable
        ...
        \ifdefstring{\board}{beagleplay}{\code{__TARGET_ARCH_arm64}}{\code{__TARGET_ARCH_arm}}.
    \item Our program needs some headers provided by libbpf. We can
        provide access to those headers to clang with the \code{--sysroot}
        parameter.
\end{itemize}
}
...
{\em TIP: if you don't see any log in your trace buffer, make sure that
tracing is enabled (tracing\_on entry in tracefs)}
