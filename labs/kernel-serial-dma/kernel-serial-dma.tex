...
The USART AND UART controllers cannot directly make use of the external DMA controller.
They have to use an intermediate DMAMUX request multiplexer that enables routing
a DMA request line between the USART/UART and the DMA controllers.
The DMA multiplexer uses the same API than the DMA controller. So we
will have to deal with the \code{dmaengine} API in order to prepare DMA
transfers on the controller side.}
\fi
...
\if\defstring{\board}{stm32mp1}{
We should at the very least request the DMA channel to use.
Open the SoC device tree and find the usart3 and uart7 nodes. Look for
\code{dma} channel properties and their names.
Hence, when requesting the channels with
\kfunc{dma_request_chan}, we must take care to check and return the error code
wrapped in the returned \kstruct{dma_chan} pointer. This can be done with the
\code{IS_ERR()} and \code{PTR_ERR()} macros. You may display the corresponding
...
\begin{verbatim}
struct dma_slave_config txconf = {};
serial->fifo_dma_addr = dma_map_resource(serial->dev, serial->res->start + USART_TDR,
                                         4, DMA_TO_DEVICE, 0);
if (dma_mapping_error(serial->dev, serial->fifo_dma_addr)) ...
\end{verbatim}
}\fi
...
unsigned int cr3 = 0;
cr3 |= USART_CR3_DMAT;
reg_write(serial, cr3, USART_CR3);
\end{verbatim}
}\fi
...
\item a \code{struct completion txcomplete} object to asynchronously inform the
  ...
  wait queue in the interrupt lab). This object shall be initialized with
