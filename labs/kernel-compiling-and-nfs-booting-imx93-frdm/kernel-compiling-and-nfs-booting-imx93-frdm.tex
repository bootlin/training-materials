...
\item Disable \kconfig{CONFIG_GCC_PLUGINS} if it is set. This will skip
  ...
  for the build.
\item In the \code{System Type} menu, remove support for all the SoCs except the \code{NXP SoC support} ones and in particular \code{NXP i.MX SoC family}.
...
\item To use ethetnet port enable as built in \kconfig{CONFIG_STMMAC_ETH}, \kconfig{CONFIG_STMMAC_PLATFORM}, \kconfig{CONFIG_NVMEM_IMX_OCOTP_ELE}, and \kconfig{CONFIG_DWMAC_IMX8}
...
nfsroot=192.168.0.1://tmp/rootfs,nfsvers=3,tcp rw ip=192.168.0.100::192.168.0.1:255.255.255.0::eth0:off'}
...
\begin{verbatim}
make savedefconfig
cp defconfig arch/arm64/configs/ix93-frdm_defconfig
\end{verbatim}
