...
channel properties and their names. Hence, when requesting the channels with
...
\end{itemize}
The LPUART\_DATA register itself should not be accessed directly. To be generic, we first map the resource using \kfunc{dma_map_resource} and store it in \code{serial_dev} to unmap it later.

\begin{verbatim}
struct dma_slave_config txconf = {};

    serial->fifo_dma_addr = dma_map_resource(serial->dev,
    serial->res->start + LPUART_DATA, 4, DMA_TO_DEVICE, 0);
...
\begin{verbatim}
  u32 val = reg_read(serial, LPUART_BAUD);
  val |= LPUART_BAUD_TDMAE;
  reg_write(serial, val, LPUART_BAUD);
\end{verbatim}
