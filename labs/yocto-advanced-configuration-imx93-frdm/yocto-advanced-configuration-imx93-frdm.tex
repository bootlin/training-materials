...
\begin{verbatim}
unzstd -c core-image-minimal-bootlinlabs.rootfs.tar.zst | sudo tar -xvf - -C /tmp
\end{verbatim}
...
\small
\begin{verbatim}
NOTE: selecting linux-fslc-imx to satisfy virtual/kernel due to PREFERRED_PROVIDERS
\end{verbatim}
\normalsize
...
from \code{linux-imx} to \code{linux-imx-xwayland} only using the
local configuration.
