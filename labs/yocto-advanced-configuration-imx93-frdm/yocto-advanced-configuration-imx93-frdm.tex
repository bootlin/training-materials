\subchapter{Lab 2: Advanced Yocto configuration}{Configure the build, customize the
...
Linux kernel at boot time. For that, we will use the U-Boot shell
to set the \code{bootargs} variable.

\begin{bashinput}
setenv bootargs 'console=ttyLP0,115200 root=/dev/nfs nfsroot=192.168.0.1:/nfs,nfsvers=3,tcp rw ip=192.168.0.100::192.168.0.1:255.255.255.0::eth0:off'
...
With a network cable, connect the \code{ENET2} Ethernet port of your board to the
one of your computer. If your computer already has a wired connection
to the network or doesn't have any Ethernet port, your instructor will
...
\section{Add a new rootfs format}

In the previous lab, we generated an SD card image containing our rootfs.
Since we will switch to NFS in the next step, a different rootfs format will be
needed.

The \yoctovar{IMAGE_FSTYPES} lists the rootfs formats generated. Go
back to \code{$BUILDDIR/conf/local.conf} and add the \code{tar.gz} format.
...
\begin{verbatim}
sudo tar xpf $BUILDDIR/tmp/deploy/images/imx93-11x11-lpddr4x-frdm/\
  core-image-minimal-imx93-11x11-lpddr4x-frdm.rootfs.tar.gz -C /nfs
\end{verbatim}

Then boot the board.

The Dropbear SSH server was enabled a few steps before, and should now be
running as a service on the imx93-frdm. You can test it by accessing the
board through SSH:
\begin{verbatim}
ssh root@192.168.0.100
\end{verbatim}

You should see the imx93-frdm command line!
...
In our case, we can see the \code{linux-imx} provides the
...
from \code{linux-imx} to \code{linux-fslc-imx} only using the
local configuration.
...
As this was only to show how to select a preferred provider for a given package, you can now use linux-imx again.
...
kernel image and the Device Tree over TFTP:

\small{
\begin{verbatim}
setenv ipaddr 192.168.0.100
setenv serverip 192.168.0.1
setenv bootcmd 'tftp ${loadaddr} Image; tftp ${fdt_addr} ${fdtfile}; booti ${loadaddr} - ${fdt_addr}'
\end{verbatim}
}
