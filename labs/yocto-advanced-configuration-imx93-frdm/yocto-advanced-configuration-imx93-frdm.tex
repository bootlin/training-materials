...
\begin{verbatim}
sudo tar xpf $BUILDDIR/tmp/deploy/images/imx93-11x11-lpddr4x-evk/\
  core-image-minimal-imx93-11x11-lpddr4x-evk.rootfs.tar.gz -C /nfs
\end{verbatim}
...
In our case, we can see the \code{linux-fslc-imx} provides the
...
from \code{linux-fslc-imx} to \code{linux-imx} only using the
local configuration.
...
setenv bootcmd 'tftp ${loadaddr} Image; tftp ${fdt_addr} dtb; booti ${loadaddr} - ${fdt_addr}'
\end{verbatim}
}
