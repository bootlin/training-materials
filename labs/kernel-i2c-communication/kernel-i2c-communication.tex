...
compiling (stay in the \code{bootlin-labs} branch).
...
\item Let the CPU wait for 1 ms by using \kfunc{fsleep} (more on sleeping
      mechanisms later in the training).\footnote{\kfunc{fsleep} is a flexible
      function that finds out what is the best internal waiting function for the
      delay requested. It will either enter a busy-wait state
      (eg. \kfunc{udelay}) or will block by entering a sleep state
      (eg. \kfunc{usleep_range} or \kfunc{msleep}).

      As a going further task, you can use Elixir to find the right C headers to
      include if you wanted to use a function like \kfunc{udelay} directly. The
      Elixir results may be a bit confusing here, because \kfunc{udelay} is
      defined in \code{arch/<arch>/include/asm/delay.h} files, and not
      in \kfile{include/linux/delay.h>} that would normally be the canonical
      place. However, look at \kfile{include/linux/delay.h} and you will see
      that it includes \code{asm/delay.h} which corresponds to the
      specific headers for the current architecture. So you would indeed need to
      include \code{linux/delay.h} anyway!

      {\bf General rule}: whenever the symbol you're looking
      for is defined in \code{arch/<arch>/include/asm/<file>.h}, you
      should include \code{linux/<file>.h} in your kernel code.
      }
...
internal bit to advertise a possible end-of-conversion status, the user
...
\begin{enumerate}
\item Start by putting a \code{10 ms} sleep. Such waiting time is
      needed to add time between the previous i2c operation and the next one.
\item Write \code{0x00} to the bus. That will allow us to read
      the device registers.
\item Add another \code{10 ms} sleep.
