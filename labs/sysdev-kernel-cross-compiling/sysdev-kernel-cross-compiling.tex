...
\if\defstring{\lab}{imx93-frdm}
Before continuing, we still need to apply a patch in kernel/data to
add the Device Tree for the \boarddescription{} in
\code{arch/arm64/boot/dts/freescale} because the board is not yet
supported in the upstream Linux kernel.

\begin{bashinput}
$ git apply ../data/0001-arm64-dts-freescale-add-support-for-NXP-i.MX93-FRDM.patch
\end{bashinput}

After applying the patch edit the Makefile to build
...
\ifdefstring{\board}{stm32mp1}{
  ...
  \item Disable \kconfig{CONFIG_DRM}, \kconfig{CONFIG_WLAN},
    ...
    support for many display, GPU, WiFi and Bluetooh drivers which saves space
    and build time.}{}
\ifdefstring{\board}{qemu}{
...
\item To use ethetnet port enable as built in
  \code{CONFIG_STMMAC_ETH},
  \code{CONFIG_STMMAC_PLATFORM},\code{CONFIG_NVMEM_IMX_OCOTP_ELE}, and
  \code{CONFIG_DWMAC_IMX8}
\item Disable \kconfig{CONFIG_DRM} and \code{CONFIG_MEDIA_SUPPORT},
