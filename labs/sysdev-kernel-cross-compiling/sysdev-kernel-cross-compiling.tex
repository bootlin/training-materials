...
\item Cross compile the kernel for the \boarddescription
...
\ifdefstring{\board}{qemu}{}{%
  \ifdefstring{\board}{beagleplay}{}{which corresponds to an LTS release, and}
}
...
\begin{bashinput}
$ git checkout linux-%\workingkernel%.y
\end{bashinput}
...
It is assumed that the \code{ARCH} and \code{CROSS_COMPILE} variables have been exported.

\section{Install dependencies}

To build the Linux kernel, you need to install the \code{bc} utility:
...
So, apply this configuration by running \code{make <default-configuration>}, and
then run \code{make menuconfig} to fine-tune it.
...
\ifdefstring{\board}{stm32mp1}{
  ...
  in a submenu.
  \item Disable \kconfig{CONFIG_DRM}, \kconfig{CONFIG_WLAN},
    \kconfig{CONFIG_MEDIA_SUPPORT} and \kconfig{CONFIG_BT}. It will remove
    support for many display, GPU, WiFi and Bluetooth drivers which saves space
    and build time.}{}
\ifdefstring{\board}{qemu}{
...
\begin{itemize}
  ...
  \item We need to enable \kconfig{CONFIG_AM335X_PHY_USB}. You will
    realize that this option is not available/accessible, so check its
    dependencies. Some of its dependencies are indeed not enabled,
    find what are those dependencies, enable them, and finally enable
...
\item In the \code{Platform Selection} menu, remove support for all the
  SoCs except the \code{NXP SoC support} ones and in particular
  \code{NXP i.MX SoC family}.
\item Select built-in support for the IMX Ethernet driver:
  \kconfig{CONFIG_DWMAC_IMX8}. Note that it depends on \kconfig{CONFIG_STMMAC_ETH}
  and \kconfig{CONFIG_STMMAC_PLATFORM}, so you need to select these two configs as
  built-in to be able to do the same with \kconfig{CONFIG_DWMAC_IMX8}.
\item Select built-in support for the fuse driver: \kconfig{CONFIG_NVMEM_IMX_OCOTP_ELE}
  This driver is used to retrieve the MAC address from the SoC fuses.
\item Disable \kconfig{CONFIG_DRM} and \kconfig{CONFIG_MEDIA_SUPPORT},
  which will disable support for display controller drivers, GPU
  drivers and camera drivers, that we don't need and will save quite a
  bit of build time.
\fi
...
optimized kernel for your board: the ARM64 \code{defconfig}
enables plenty of features and drivers that will not be useful on our
particular board.}{}
