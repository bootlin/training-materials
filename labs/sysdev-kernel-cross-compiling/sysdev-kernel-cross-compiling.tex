...
\begin{bashinput}
$ export PATH=$HOME/x-tools/%\ifdefstring{\board}{beagleplay}{aarch64-training-linux-musl}{arm-training-linux-musleabihf}%/bin:$PATH
\end{bashinput}
...
\item On your workstation, copy the {\tt \ifdefstring{\board}{beagleplay}{Image.gz}{zImage}} and DTB
(\texttt{\dtname}\texttt{.dtb}) to the directory exposed by the TFTP server.

\item On the target (in the U-Boot prompt), load {\tt \ifdefstring{\board}{beagleplay}{Image.gz}{zImage}} from
TFTP into RAM:
\begin{ubootinput}
=> tftp %\zimageboardaddr% %\ifdefstring{\board}{beagleplay}{Image.gz}{zImage}%
\end{ubootinput}
...
\begin{ubootinput}
=> %\ifdefstring{\board}{beagleplay}{booti}{bootz} \zimageboardaddr% - %\dtbboardaddr%
\end{ubootinput}
