...
\item \code{Target packages} menu
  ...
  add the below options:
     \begin{itemize}
     \item Select \code{Enable GPL code}
     \item Select \code{Build libavresample}
     \item Select \code{Build libpostproc}
...
cp ~/boot-time-labs/data/S50playvideo board/beaglecam/rootfs-overlay/etc/init.d/
\end{verbatim}
