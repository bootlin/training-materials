...
\item In our case, the first stage bootloader is U-Boot SPL, and it
  ...
  additional closed-source firmware files from the DDR controller
...
\end{bashinput}
Accept the conditions, it will unpack the needed binaries under
\code{firmware-imx-8.21/firmware/ddr/synopsys/}.
...
\begin{itemize}
\item The cross-compiler prefix (the part before gcc in the
  cross-compiler executable name), set through the \code{CROSS_COMPILE}
  variable. The same \code{CROSS_COMPILE} variable will be used later by U-Boot,
  so instead of setting it in the \code{make} command, you can directly export it
  into your shell environment with \inlinebash{$ export CROSS_COMPILE=aarch64-linux-}
  \item The used platform: \code{PLAT=imx93}
\end{itemize}
...
If you're using a B2 revision of the IMX93-FRDM, you'll need to cherry-pick
commit \code{7353fadd381 ("imx93_frdm: Add support for 2CS 2GB DRAM support")}
that adds support for the DDR used on the B2. This commit has not yet entered
any stable release so we have no choice but to cherry-pick it from u-boot/master:
\begin{bashinput}
$ git cherry-pick 7353fadd381
\end{bashinput}

Load the iMX93-FRDM default configuration with
\inlinebash{$ make imx93_frdm_defconfig}, it will load the configuration
described in \code{configs/imx93_frdm_defconfig}

Now that you have a valid initial configuration, you can now run
...
Run \inlinebash{make} to build U-Boot \footnote{You can speed up the compiling
by using the \code{-jX} option with \code{make}, where X is the number of
parallel jobs used for compiling. Twice the number of CPU cores is a good value.}.
Note that the default device tree selected by \code{imx93_frdm_defconfig} fits
our need, so we don't need to set the \code{DEVICE_TREE} variable in the
\code{make} command.

The build fails at the BINMAN step with following error log:
\begin{terminaloutput}
  BINMAN  .binman_stamp
file './mx93a1-ahab-container.img' does not exist. flash.bin may be not-functional.
file './bl31.bin' does not exist. flash.bin may be not-functional.
...
To build the final \code{flash.bin}, U-Boot needs the BL31 binary (i.e. TF-A)
and the binaries used during the DDR initialization. The location of these
binaries is configured in the \code{binman} node of the device tree (see
\code{arch/arm/dts/imx93-u-boot.dtsi}). The current configuration expects them
to be in U-Boot's top directory so let's copy them here:
\begin{bashinput}
$ cp ../trusted-firmware-a/build/imx93/release/bl31.bin .
$ cp ../firmware-imx-8.21/firmware/ddr/synopsys/lpddr4_dmem_1d_v202201.bin .
$ cp ../firmware-imx-8.21/firmware/ddr/synopsys/lpddr4_imem_1d_v202201.bin .
$ cp ../firmware-imx-8.21/firmware/ddr/synopsys/lpddr4_imem_2d_v202201.bin .
$ cp ../firmware-imx-8.21/firmware/ddr/synopsys/lpddr4_dmem_2d_v202201.bin .
\end{bashinput}

Now run \code{make} again, the build should end sucessfully. The warning about
the missing ./mx93a1-ahab-container.img can be ignored as we don't implement
secure boot.
...
The ROM code of the NXP i.MX93 doesn't support any filesystem. It expects
to find the raw \code{flash.bin}, at offset 32 KiB of the storage medium. Though,
during the U-Boot configuration, we configured the U-Boot environment to be
stored in an EXT4 partition. So, we're going to create a single partition to
store this environment. The starting offset of this partition will be 2 MB to
leave enough room for \code{flash.bin}.
...
Then, the one partition, from offset 2 MiB to offset 34 MiB, will be
created using:

\begin{verbatim}
(parted) unit mib
(parted) mkpart bootfs ext4 2 34
\end{verbatim}
...
Number  Start    End       Size      File system  Name    Flags
 1      2.00MiB  34.0MiB   32.00MiB  ext4         bootfs
...
Ensure the Boot Mode switch (ON 57) is configured to boot from the SD card:
[1100].
...
\begin{verbatim}
U-Boot SPL 2026.07-00001-gbe857fe99f32 (Jul 15 2026 - 16:55:57 +0200)
...
Load image from 0x3d800 by ROM_API
NOTICE:  TRDC init done
NOTICE:  BL31: v2.15.0(release):v2.15.0
NOTICE:  BL31: Built : 16:48:10, Jul 15 2026


U-Boot 2026.07-00001-gbe857fe99f32 (Jul 15 2026 - 16:55:57 +0200)
...
board download files, such as the kernel image and the Device Tree Binary
...
\code{/etc/default/tftpd-hpa}, and check what the TFTP server home directory is
