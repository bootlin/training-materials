\subchapter{Lab 6: Create a custom machine configuration}{Let Poky know about your
  hardware!}
...
what we did in lab1, when we chose the \board{} one.
...
will make the \board\ properly boot.

\if\defstring{\board}{imx93-frdm}
This machine describes a board using the \code{imx93} SoC and we take the board
\code{imx93-evk} as a base. Add the following line to your machine configuration file:
\begin{verbatim}
...
\end{verbatim}
  ...
  \item We also need to mark this machine as compatible with both the Linux and
		U-Boot recipes we are using:
\begin{verbatim}
COMPATIBLE_MACHINE:append:pn-${PREFERRED_PROVIDER_virtual/kernel} = "|bootlinlabs"
COMPATIBLE_MACHINE:append:pn-${PREFERRED_PROVIDER_virtual/bootloader} = "|bootlinlabs"
\end{verbatim}
