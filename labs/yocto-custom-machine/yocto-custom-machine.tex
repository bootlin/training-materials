...
This machine describes a board using the \code{imx93} SoC et nous prenons comme base la carte \code{imx93-evk}. Add the following line to your machine configuration file:
\begin{verbatim}
...
\if\defstring{\labboard}{imx93-frdm}
\begin{itemize}
  \item On doit ajouter les firmware de nxp pour l'initialisation de la memoire DDR 
\begin{verbatim}
DDR_FIRMWARE_NAME = " \
...
\end{verbatim}
  ...
  \item On doit egalement préciser qu'elle recette kernel choisir,qu'elle device tree le kernel doit utiliser, On ajoute le que l'on veux un rootfs de type tar. On precise le type d'image kernel que l'on veux:
\begin{verbatim}
...
\end{verbatim}
  ...
  \item on doit egalement doit presiser les meme elements pour u-boot.
\begin{verbatim}
