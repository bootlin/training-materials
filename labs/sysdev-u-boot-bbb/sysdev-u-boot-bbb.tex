...
\bashcmd{$ export CROSS_COMPILE=arm-training-linux-musleabihf-}
...
card, you need to hold the \code{USER(S2)} button close to the USB host
...
Let's install a TFTP (Trivial File Transfer Protocol) server on your development workstation:
...
If the loading was not succesfull, e.g.:
\begin{verbatim}
...
\end{verbatim}
make sure that the TFTP port(\code{69} by default) is open on the firewall.
It is advised to open all ports for communication, since there will be other servers involved:
\begin{verbatim}
