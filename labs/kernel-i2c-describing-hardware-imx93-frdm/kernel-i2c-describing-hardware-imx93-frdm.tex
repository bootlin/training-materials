...
Go to the \code{~/linux-kernel-imx93-frdm-labs/src/linux} directory. Check out the
\code{6.15.bootlin} branch.
Now create a new \code{imx93-custom} branch starting from this branch,
for your upcoming Device Tree changes on the imx93-frdm board.
...
As the imx93-frdm device tree is provided by the kernel community,
...
system. Set the \code{model} property to \code{Training imx93-frdm}. Don't hesitate to ask your instructor if you're not sure how.
...
The imx93-frdm board has a RGB LED on the side of the debug USB-C port.But it does not light up.
...
the in-use Device Tree, the LED remain off. The reason for that is the
absence of driver for this node: nothing actually drive the LED even
...
You should now see the LED bright in white. If you look at the bindings documents
...
notice we can tweak the \code{default-state} in order to change the color of the LED to blue.

You will need to modify the DTS file in order to do that. But
because we do not want to impact the orginal file, we might instead add a label to the \code{leds} container node. We
...
As we can see in the board reference manual, the i.MX93-FRDM board has only one I2C bus available on its expansion connectors. This is LPI2C1, which is connected to the 10-pin 2x5 2.54 mm connector. But if we look at the design files, we can see that the SoC's LPI2C4 bus is connected to \code{EXP_GPIO_IO02} and \code{EXP_GPIO_IO03} of the expansion connector.

From a training perspective, we will enable bus lpi2c4, which is disabled by default in the board's Device Tree.
...
Here you can see there is only 3 i2c .
...
Now let's double check the addressings by looking at the imx 93 processor reference manual, downloaded earlier.
