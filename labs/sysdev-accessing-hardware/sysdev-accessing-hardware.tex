...
Don't hesitate to export \code{/dev} on your workstation too
...
\code{gpio65} file should appear in \code{/sys/class/gpio}.
...
\code{/sys/kernel/debug/gpio} again, and see that \code{gpio-65} is now
...
Go to \code{/sys/class/leds} to see all the LEDs that you are allowed
to control.

Currently, on the DK1 board, at least with the default Device Tree and
kernel configuration, you can only control the \code{heartbeat} LED.
...
\code{STM32MP157A-DK1 media player}. Don't hesitate to ask your
