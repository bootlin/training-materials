...
that ext4 and SquashFS are supported. If that's not the case, recompile
your kernel with support for SquashFS and ext4\footnote{Basic configuration
options for these filesystems will be sufficient. No need for things like
extended attributes.}.
You can also check the already existing partitions on your system by looking
into \code{/proc/partitions}. The several mmcblk0* partitions you'll see are
the come from the eMMC of the FRDM.

\section{Add partitions to the SD card}

Plug the SD card in your workstation.

Let's use \code{parted} to create two new partitions:
\begin{itemize}
\item A second primary partition, 100 MB big, for the root filesystem
...
\begin{verbatim}
(parted) unit mib
(parted) mkpart rootfs 34 134
(parted) mkpart data 134 100%
(parted) print
...
Number  Start    End       Size      File system  Name    Flags
 ...
 2      34.0MiB  134MiB    100MiB                 rootfs
 3      134MiB   15193MiB  15059MiB               data
...
Unmount the SD card, insert it in your board and boot. You should see the
new partitions in \code{/proc/partitions}.
...
Insert the SD card in your PC. Then copy the kernel and device tree into the
first partition, next to the U-Boot environment.
...
\begin{ubootinput}
=> load mmc 1:1 %\zimageboardaddr% Image.gz
=> load mmc 1:1 %\dtbboardaddr% %\dtname%-custom.dtb
