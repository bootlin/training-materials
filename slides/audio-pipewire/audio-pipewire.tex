...
\begin{frame}{Tools rundown — \code{pw-top}}
  ...
  \begin{center}
    \includegraphics[height=0.5\textheight]{slides/audio-pipewire/pw-top.png}\\
  \end{center}
\end{frame}
...
\begin{frame}{WirePlumber — default behavior}
  ...
  \item Nodes can also request to be routed to:
    ...
    \item a target node using \code{target.object} inside \code{default}
      metadata (at runtime).
    \end{enumerate}
...
\begin{frame}{Going further}
  ...
  \item The \code{pipewire-aes67} binary symlinks to \code{pipewire}
    and loads config \code{pipewire-aes67.conf}. See
    \href{https://gitlab.freedesktop.org/pipewire/pipewire/-/wikis/AES67}{wiki}
    and \href{https://gitlab.freedesktop.org/pipewire/pipewire/-/blob/master/src/daemon/pipewire-aes67.conf.in}{configuration}.
