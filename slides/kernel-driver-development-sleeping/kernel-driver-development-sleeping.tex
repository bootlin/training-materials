...
\begin{frame}
  ...
  \begin{center}
    ...
    Sleeping is needed when a userspace or kernelspace thread is
    waiting for data.
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{How to sleep with a wait queue 2/3}
  Several ways to make a thread sleep
  \begin{itemize}
  \item \mint{c}+void wait_event(queue, condition);+
    \begin{itemize}
    \item Sleeps until the wait queue is woken up {\bf and} the given C
      expression is true. Caution: once entered, the thread sleep
...
\begin{frame}[fragile]
  \frametitle{How to sleep with a wait queue 3/3}
  \begin{itemize}
  \item \mint{c}+int wait_event_timeout(queue, condition, timeout);+
    \begin{itemize}
    \item Also stops sleeping when the thread is woken up {\bf or} the
...
\begin{frame}[fragile]
\frametitle{How to sleep with a wait queue - Example}
\begin{minted}{c}
...
\begin{frame}
  ...
  threads are waiting for become available.
  \begin{itemize}
  \item \code{wake_up(&queue);}
    \begin{itemize}
    \item Wakes up all threads in the wait queue
    \end{itemize}
  \item \code{wake_up_interruptible(&queue);}
    \begin{itemize}
    \item Wakes up all threads waiting in an interruptible sleep
      on the given queue
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item \kfunc{wait_event_interruptible} puts a thread in a
    non-exclusive wait.
    \begin{itemize}
    \item All non-exclusive threads are woken up by \kfunc{wake_up} /
      \kfunc{wake_up_interruptible}
    \end{itemize}
  \item \kfunc{wait_event_interruptible_exclusive} puts a thread in an
    exclusive wait.
    \begin{itemize}
    \item \kfunc{wake_up} / \kfunc{wake_up_interruptible} wakes up
      all non-exclusive threads and only one exclusive thread
    \item \kfunc{wake_up_all} / \kfunc{wake_up_interruptible_all}
      wakes up all non-exclusive and all exclusive threads
    \end{itemize}
  \item Exclusive sleeps are useful to avoid waking up multiple threads
    when only one will be able to \textit{consume} the event.
  \item Non-exclusive sleeps are useful when the event can
    \textit{benefit} to multiple threads.
  \end{itemize}
\end{frame}
  ...
  \begin{columns}
    ...
    \column{0.5\textwidth}
       ...
       \item \mint{c}+wait_event(queue, cond);+
         The thread is put in the \ksym{TASK_UNINTERRUPTIBLE} state.
       \item \mint{c}+wake_up(&queue);+
         All threads waiting in \code{queue} are woken up, so they get
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item Enter a wait state with \mint{c}+void wait_for_completion(struct completion *done)+
    \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \item Use helpers which implement software loops or use hardware timers
    \begin{itemize}
    \item \kfunc{udelay} waste CPU cycles in order to save a couple of
      context switches, suitable for $\le$ 10us or in atomic situations
    \item \kfunc{usleep}/\kfunc{usleep_range}/\kfunc{msleep} put the
      thread in sleep for a given amount of micro/milliseconds
      (not suitable in atomic contexts)
    \item If in doubt, use \kfunc{fsleep}, which will use the more suitable
