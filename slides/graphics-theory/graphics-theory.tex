...
\begin{frame}{Alpha blending}
  \begin{itemize}
  \item Various alpha-enabled pixel sources are composited with alpha blending
  \item Results in a single picture aggregating sources
    \begin{itemize}
    \item Sources can be stacked with a priority (z-order)
    \item Semi-transparent surfaces need to be merged together
    \end{itemize}
  \item Different alpha merging operations were defined by Porter and Duff:
  \end{itemize}
  \begin{center}
...
\begin{frame}{Linear filtering and convolution}
  ...
  \item The pixel value is a linear sum of weighted neighboring values
\[
o_{x,y} = \alpha_0 i_{x,y} + \alpha_1 i_{x-1,y} + \alpha_2 i_{x+1,y} + ...
\]
  \item Weighting coefficients are represented in a 2D matrix, the filter kernel
  \item Corresponds to a Convolution operation with the filter kernel \(\omega\):
\[
g(x,y)= \omega *f(x,y)=\sum_{s=-a}^a{\sum_{t=-b}^b{ \omega (s,t)f(x-s,y-t)}}
\]
  \item Runs for each pixel: optimizations are implemented
  \item Very useful for 2D signal treatment
  \end{itemize}
\end{frame}

\begin{frame}{Linear filtering and convolution (illustrated)}
  \begin{center}
  \includegraphics[width=0.4\textwidth]{slides/graphics-theory/convolution.png}\\
  \textit{\small Linear filtering with the convolution operation}
  \end{center}
\end{frame}
