...
\begin{frame}
  ...
  \begin{itemize}
  \item Splitting the execution of interrupt handlers in 2 parts is sometimes needed
    \begin{itemize}
    \item Hard IRQ handlers execute with all interrupts on the local CPU masked
      \begin{itemize}
      \item No interrupt nesting mechanism, higher priority IRQs will be delayed
      \end{itemize}
    \item Need to block/sleep (eg. accessing I2C devices may involve sleeping)
    \end{itemize}
  \item Top half
    \begin{itemize}
    \item This is the real interrupt handler, which should complete as quickly
      as possible since all interrupts are disabled.  It takes the data out of
      the device and if substantial post-processing is needed, schedules a
      bottom half to handle it.
    \end{itemize}
  \item Bottom half
    \begin{itemize}
    \item Is the general name for various mechanisms which allow to postpone the
      handling of interrupt-related work.
      \begin{itemize}
      \item Implemented in Linux as softirqs, threaded handlers and workqueues
      \item And yet, the abbreviation "bh" often means "softirqs" for historical
        reasons!
      \end{itemize}
    \end{itemize}
  \end{itemize}
...
\begin{frame}
  \frametitle{Softirqs}
  \begin{itemize}
  \item Softirq handlers are callbacks executed once all interrupt handlers have
    completed, before the kernel resumes scheduling processes
    \begin{itemize}
    \item They execute with all interrupts enabled
    \item They run before the scheduler is in control, so sleeping is not allowed
    \item A softirq handler can run simultaneously on multiple CPUs
    \end{itemize}
  \item The number of softirqs is fixed, softirqs are not used by drivers, but
    by kernel subsystems (network, etc.). The list of softirqs is defined in
    \kfile{include/linux/interrupt.h}
  \item To avoid starving the system, there is typically a softirq budget
    (by default, softirqs can run 10 times in a row for a maximum time of 2ms)
    after which the callbacks are run in process context in a per CPU
    \code{ksoftirqd/N} kernel thread.
  \item A Tasklet is a deprecated mechanism which was dedicated to device
    drivers and was implemented on top of softirqs.
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Softirq execution flow}
  \begin{center}
...
\begin{frame}
  ...
  \item Interface in the Linux kernel used for interrupt mitigation in
        network drivers
  \item Principle: when the network traffic exceeds a given threshold
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item It is possible to associate a threaded handler to a hard IRQ handler
    \begin{itemize}
    \item The hard IRQ handler triggers it by returning \ksym{IRQ_WAKE_THREAD}
    \item The threaded handler is executed inside a thread, in process context
    \end{itemize}
  \item The interrupt line may be kept masked if the handler registration
    happened with \ksym{IRQF_ONESHOT}
  \item Sleeping/blocking is allowed in the threaded handler
    \begin{itemize}
    \item The priority of the \code{irq/<nb>-<name>} thread can be tuned!
    \end{itemize}
  \item Heavily used by \code{PREEMPT_RT}
  \end{itemize}
  \begin{minted}{c}
...
\begin{frame}[fragile]
  ...
  \item Workqueues are a general mechanism for deferring work. It is
    ...
    \begin{itemize}
    \item Typically used for background jobs.
    \end{itemize}
...
\begin{frame}
  \frametitle{Interrupt and deferred mechanisms execution constraints summary}
  \begin{center}
    \small
    \begin{tabular}{| l || c | c | c | c | c |}
\hline
\textbf{Mechanism} & \textbf{Context} & \textbf{IRQs} & \textbf{Priority} & \textbf{Can}   & \textbf{Typical} \\
                   &                  &               & \textbf{tuning}   & \textbf{sleep} & \textbf{Use} \\
\hline
Hard IRQ & Interrupt & Disabled    & No,  & No & Fast handling, \\
         &           & (local CPU) & FIFO &    & gen. purpose \\
\hline
Softirq & Interrupt & Enabled & No,              & No & Fast handling, \\
        &           &         & fixed priorities &    & net, timers, RCU \\
\hline
Softirq & Process     & Enabled & One kthread & No & Softirq \\
        & (ksoftirqd) &         & per CPU     &    & overflow \\
\hline
Threaded IRQ & Process      & Enabled               & Yes & Yes & Interrupt handling \\
             & (irq thread) & (\ksym{IRQF_ONESHOT}) &     &     & needing to block \\
\hline
Workqueue & Process  & Enabled & Yes,         & Yes & General purpose \\
          & (worker) &         & per queue    &     & background worker \\
\hline
    \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \item Interrupt handler
    ...
    \item If needed, trigger a bottom half mechanism (typically taking care of
      handling data)
