...
\begin{frame}
  ...
  \begin{itemize}
	...
	\item Versions \code{6.0} (Oct. 2022) to \code{6.19} (Feb. 2026)
	\item Version \code{7.0} will be released in March/April 2026.
  \end{itemize}
  ...
  \item Major updates \href{https://lore.kernel.org/lkml/CAHk-=wiiRA_XxoF96Q_1n4BadBGJLRkHarHG92u3aTc+1ZMeGQ@mail.gmail.com/}{have no specific meaning}! \\
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \item Solution: the last release of each year is made an LTS {\em (Long Term
    Support)} release, and is supposed to be supported (and receive bug and
    security fixes) for at least 2 years. These projected EOL may be extended up
    to 6 years based on the industry interest at large.
  \begin{columns}
  \column{0.6\textwidth}
...
\begin{frame}[fragile]
  ...
  \item The {\em Civil Infrastructure Platform} project is an industry /
    ...
    selected LTS versions (currently 4.4, 4.19, 5.10, 6.1 and 6.12) on selected architectures.
...
% in the Linux kernel source directory. It gives you the header line
% (# of files, lines and size), the list of directories and the root files.
\begin{frame}
  \frametitle{Linux kernel size and structure}
  \begin{itemize}
  \item Linux v6.19 sources: close to 92k files, 42M lines, 1.4GiB
  ...
  \item As of kernel version v6.19 (in percentage of total number of lines):
  \end{itemize}
  {\small
  \begin{columns}
    \column[t]{0.31\textwidth}
    \begin{itemize}
    \item \kdir{drivers}: 61\%
    \item \kdir{arch}: 12\%
    \item \kdir{tools}: 5.4\%
    \item \kdir{sound}: 3.9\%
    \item \kdir{fs}: 3.8\%
    \item \kdir{Documentation}: 3.8\%
    \end{itemize}
    \column[t]{0.25\textwidth}
    \begin{itemize}
    \item \kdir{include}: 3.4\%
    \item \kdir{net}: 3.1\%
    \item \kdir{kernel}: 1.3\%
    \item \kdir{lib}: 0.8\%
    \item \kdir{mm}: 0.5\%
    \item \kdir{scripts}: 0.3\%
    \end{itemize}
    \column[t]{0.50\textwidth}
    \begin{itemize}
    \item \kdir{security}, \kdir{rust}, \kdir{crypto}, \kdir{block},
      \kdir{samples}, \kdir{io_uring}, \kdir{virt}, \kdir{usr},
      \kdir{LICENSES}, \kdir{ipc}, \kdir{init}, \kdir{certs}: $\leq$0.3\%
