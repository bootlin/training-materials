...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Some recipes may provide the same functionality, e.g.
...
COLOUR ?= "unknown"
  ...
  \item The \code{??=} operator assigns a value only if the variable has
    ...
    \code{?=} operator
  \item The \href{https://docs.yoctoproject.org/bitbake/bitbake-user-manual/bitbake-user-manual-metadata.html#setting-a-default-value}{BitBake documentation}
...
\subsection{Virtual providers}

\begin{frame}
  \frametitle{Introduction to virtual providers}
  \begin{itemize}
    \item Some recipes have the same purpose, and only one can be
      used at a time.
    \item The build system uses {\bf virtual providers} to reflect
      this.
    \item Only one of the recipes that provides the functionality will
...
\begin{frame}
  ...
  \begin{itemize}
    \item The virtual provider names are often in the form
      \code{virtual/<name>}
    \item Example of available virtual providers with some of their
      variants:
      \begin{itemize}
...
\begin{frame}
  \frametitle{Provider selection}
  \begin{itemize}
    \item Providers are selected thanks to the
      \yoctovar{PREFERRED_PROVIDER} configuration variable.
    \item The recipe names {\bf have to} suffix this variable.
    \item Examples:
    \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item By default, Bitbake will try to build the recipe with the
      highest version number, from the highest priority layer, unless the recipe defines
      \code{DEFAULT_PREFERENCE = "-1"}
    \item When multiple recipe versions are available, it is also
      possible to explicitly pick one with
...
\begin{frame}
  ...
  \begin{itemize}
    \item Building recipes will result in binary packages being generated.
