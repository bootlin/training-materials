...
ret = ioctl(drm_fd, DRM_IOCTL_SET_CLIENT_CAP, &client_cap);
  \end{minted}
  \item Driver and client capabilities defined in Linux's \code{include/uapi/drm/drm.h}
  \end{itemize}
\end{frame}
...
ret = ioctl(fd, DRM_IOCTL_DROP_MASTER, NULL);
  \end{minted}
...
ret = ioctl(fd, DRM_IOCTL_GET_MAGIC, &auth);
  \end{minted}
  ...
  \begin{minted}[fontsize=\small]{console}
ret = ioctl(fd, DRM_IOCTL_AUTH_MAGIC, &auth);
  \end{minted}
  \end{enumerate}
...
\begin{frame}[fragile]{DRM KMS dumb buffers}
  \begin{itemize}
  \item The \textbf{dumb buffer} generic API exposes memory operations:
    \begin{itemize}
