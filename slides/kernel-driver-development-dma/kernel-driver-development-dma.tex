...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item Any device capable of being master on a bus shall assert its addressing
    capabilities.
  \item The default addressing capability is assumed to be 32-bit.
  \item If the platform supports it, the DMA addressing capability can be:
    ...
    \item decreased (eg. ISA devices, where \kfunc{kmalloc} buffers can
      ...
      \ksym{GFP_DMA})
    \item Linux stores this capability in a per-device mask
    \end{itemize}
  \item DMA mappings can fail because a buffer is out of reach
    \begin{itemize}
    \item \code{swiotlb} may solve some situations by leveraging internal
      bounce buffers
    \end{itemize}
