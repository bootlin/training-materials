...
\begin{frame}{Wayland overview and paradigm}
  ...
  \item Improves \textbf{security} aspects:
    ...
    \item Only the compositor can access display buffers\\
      \textit{only the compositor can provide screenshots}
...
% registry exposes available interfaces
\end{frame}
...
\begin{frame}{Wayland core protocol: global object interfaces}
  ...
  \item Global object interfaces are bound with the registry before use\\
    \textit{\code{wl_registry_bind} returns a pointer to a proxy object}
...
just what it allows, no details:
- where to find them
- how they are generated/distributed
- time presentation protocol
- XDG
- IVI-shell
\end{frame}

\begin{frame}{Wayland asynchronous interface}
% A slide about async: polling, flushing, etc
bla
\end{frame}
...
foo
% Talk about XWayland here
% Major compositor libraries, constants about compositors: using libinput
\end{frame}

\begin{frame}{Weston code structure and walkthrough}
% esp tests and clients
bar
\end{frame}

\begin{frame}{Wayland debug and documentation}
% weston-info lists global object interfaces
% WAYLAND_DEBUG=1
foo
\end{frame}
