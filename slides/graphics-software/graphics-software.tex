...
\begin{frame}{Wayland asynchronous interface}
  \begin{itemize}
  \item Wayland does not send requests on its own but buffers them
  \item Buffered data is sent with \code{wl_display_flush} (non-blocking)
  ...
  \item \code{wl_display_dispatch} dispatches and tries a blocking read on empty queue
  \item \code{wl_display_get_fd} returns a file descriptor for the display
    \begin{itemize}
    \item Can be polled directly with \code{wl_display_poll}
    \item Main-loop \code{poll} integration is also possible
    \item Calling \code{wl_display_dispatch} after polling is not thread-safe!\\
      \textit{see the \code{wl_display_read_events} documentation}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Wayland status and adoption}
  ...
  \item X11 applications can be integrated using \textbf{XWayland}
    ...
    \item Creates a \code{wl_surface} for each window, redirects output
    \end{itemize}
...
\begin{frame}{Standardized 3D rendering APIs: OpenGL}
  \begin{center}
  \includegraphics[height=4em]{slides/graphics-software/opengl-logo.pdf}
  \end{center}

  \begin{itemize}
  \item 3D rendering API, optimized for GPU \textbf{hardware acceleration}
    \begin{itemize}
    ...
    \end{itemize}
  \item OpenGL version 1 targeted fixed-function pipeline GPUs
  \item With version 2 and up, vertex and fragment shaders can be programmed
    \begin{itemize}
    \item Specifies the \textbf{GL Shading Language} (GLSL)
    \item C-like source syntax with intrinsic functions (e.g. texture access)
    \item Compiled on-the-fly by the GL implementation
    \end{itemize}
  \item Next versions added more features and more types of shaders (\textit{geometry, tesselation})
  \item \textbf{Stateful} and \textbf{context-based} programming model
  \item Supports \textbf{extensions} that can be queried, for extra rendering features
  \end{itemize}
\end{frame}

\begin{frame}{Standardized 3D rendering APIs: OpenGL ES and EGL}
  \begin{minipage}[t]{0.49\textwidth}
    \centering
...
\begin{frame}{Standardized 3D rendering APIs: Vulkan}
  \begin{center}
