...
\begin{frame}{Graphics Processing Unit}
  ...
  \item Operates on 3D graphics primitives: \textbf{points (vertices), lines and triangles}
    \begin{itemize}
      \item Rendered to 2D space (rasterization) with \textbf{textures} and \textbf{lighting}
      \item Not photo-realistic (not using ray tracing)
    \end{itemize}
...
\begin{frame}{Graphics Processing Unit techniques}
  \begin{itemize}
  \item \textbf{Interpolation} is key to apply textures/gradients in 3D space
  \item \textbf{Texture sampling} can easily cause aliasing (at a distance)
    \begin{itemize}
    \item \textbf{Anisotropic filtering} provides good visual results
  ...
  \item \textbf{Normals mapping/bump maps} provide increase details with low vertex count\\
  ...
  \item Using a \textbf{depth/z buffer} to keep track of the z-order for each output pixel\\
    \textit{other internal buffers can be involved to accelerate rendering}
  \end{itemize}
\end{frame}
...
\begin{frame}{Graphics Processing Unit internals}
  \begin{itemize}
  \item Texure access is provided by a \textbf{Texture Mapping Unit} (TMU)
    \begin{itemize}
...
\begin{frame}{Graphics bottleneck and offloading}
  \begin{itemize}
  \item One major bottleneck in graphics pipelines is memory access:
    \begin{itemize}
    \item Memory copies must be avoided at all costs (zero-copy)
  ...
  \item Offloading is not (always) a magical solution:
    ...
    \end{itemize}
  \item However, offloading is often crucially needed:
    \begin{itemize}
