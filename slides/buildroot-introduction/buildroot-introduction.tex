...
\begin{frame}{Buildroot at a glance}
  ...
  \item Active community, regular releases
    \begin{itemize}
    \item The present slides cover {\em Buildroot 2025.02}. There may
...
\begin{frame}{Getting Buildroot}
  ...
  \item However, it is generally more convenient to clone the Git
    ...
    \item \code{git clone https://gitlab.com/buildroot.org/buildroot.git}
    \item Git tags available for every stable release.
    \end{itemize}
  \item {\bf Long term support} releases
    \begin{itemize}
    \item Previously: YYYY.02 releases maintained one year
    \item New policy starting with {\em 2025.02}
    \item Goal is to maintain it during 3 years
    \item Security fixes, bug fixes, build fixes
    \item LTS initiative to fund the maintenance work, sponsoring
      needed to cover the effort
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Build results}
  ...
  \item There is no standard way to install the images on any given
    ...
    \item Buildroot provides some tools to generate SD card / USB key
      ...
      platforms: SAM-BA for Microchip, {\em uuu} for NXP i.MX,
      OpenOCD, etc.
    \end{itemize}
  \end{itemize}
\end{frame}
