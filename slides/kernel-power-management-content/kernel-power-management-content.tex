...
\begin{frame}
  ...
  \begin{itemize}
  \item The clock framework API and the \kstruct{clk} structure are
    usually implemented by each architecture (code duplication!)
    \begin{itemize}
    \item See \kpath{arch/arm/mach-at91/clock.c} for an example
  ...
  \item Drivers can then use the clock API. Example from
    \kpath{drivers/net/macb.c}:
    \begin{itemize}
    \item \kfunc{clk_get} called from the \code{probe()} function, to
      get the definition of a clock for the current board, get its
      frequency, and run \kfunc{clk_enable}.
    \item \kfunc{clk_put} called from the \code{remove()} function to
...
From \kpath{arch/arm/mach-at91/clock.c}: (2.6.36)
