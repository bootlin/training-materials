...
\begin{frame}
  ...
  \begin{itemize}
    \item \yoctovar{TARGET_ARCH}: The architecture of the device being built.
    \item \yoctovar{PREFERRED_PROVIDER}\code{_virtual/kernel}: The default kernel.
    \item \yoctovar{MACHINE_FEATURES}: List of hardware features provided by the
      machine, e.g. \code{usbgadget usbhost screen wifi}
    \item \yoctovar{SERIAL_CONSOLES}: Speed and device for the serial consoles to
	    attach. Used to configure \code{getty},
      e.g. \code{115200;ttyS0}
    \item \yoctovar{KERNEL_IMAGETYPE}: The type of kernel image to build, e.g.
      \code{zImage}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item \yoctovar{SPL_BINARY}: If an SPL is built, describes the name of the
      output binary. Defaults to an empty string.
    \item \yoctovar{UBOOT_SUFFIX}: \code{bin} (default) or \code{img}.
    \item \yoctovar{UBOOT_MACHINE}: The target used to build the configuration.
    \item \yoctovar{UBOOT_ENTRYPOINT}: The bootloader entry point.
    \item \yoctovar{UBOOT_LOADADDRESS}: The bootloader load address.
    \item \yoctovar{UBOOT_MAKE_TARGET}: Make target when building the
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item The kernel used is selected in the machine file thanks to:
      \yoctovar{PREFERRED_PROVIDER}\code{_virtual/kernel}
    \item Its version is defined with:
      \yoctovar{PREFERRED_VERSION}\code{_<kernel_provider>}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Automatically applies configuration fragments listed in
          \yoctovar{SRC_URI} with a \code{.cfg} extension
      \begin{block}{}
        \begin{minted}{sh}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Two main configuration variables help taking advantage of
      this:
      \begin{itemize}
        \item \yoctovar{LINUX_KERNEL_TYPE}: \code{standard} (default),
          ...
          \begin{itemize}
            ...
            \item \code{preempt-rt}: applies the \code{PREEMPT_RT}
              patch.
          \end{itemize}
        \item \yoctovar{KERNEL_FEATURES}: List of features to enable. Features
