...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item BitBake parses each layer specified in \yoctovar{BBLAYERS} and
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Other layers offer to support applications not available in
      OpenEmbedded-Core:
    \begin{itemize}
      \item \code{meta-firefox}: Firefox web browser support.
      \item \code{meta-filesystems}: support for additional
        filesystems.
      \item \code{meta-java}: Java support.
      \item \code{meta-arm-toolchain}: Arm GCC toolchain recipes.
      \item \code{meta-qt6}: QT6 modules.
      \item \code{meta-realtime}: real time tools and test programs.
      \item \code{meta-virtualization} and many more\dots
    \end{itemize}
    Notice that some of these layers do not come with all the Yocto
    branches. \code{meta-realtime} layer does not have a \code{honister}
    (3.4) branch, for example.
  \end{itemize}
\end{frame}

\subsection{Layer recommendations}
...
\begin{frame}
  ...
  \item A working example:
    ...
    \item Minimal dependencies: BitBake, OE-core, meta-arm
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Use \yoctovar{LAYERDEPENDS} to explicitly define layer
      dependencies.
    \item Use \yoctovar{LAYERSERIES_COMPAT} to define the Yocto version(s)
